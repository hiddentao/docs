...
You can also publish a subfolder of static files and rewrite the route to appear at the root of the deployment. For example, a static folder called `public` might contain several CSS files. Use the following to deploy the `public` folder contents recursively:
...
Without the `routes` entry above, the static CSS files would be located at `/public/style.css` instead of the expected`/style.css`. This route rewrite above uses a regular express to capture wildcard path and route to the matching file inside the `public` directory. The [Routes documentation](/docs/v2/deployments/routes) has more information on routing configuration.
